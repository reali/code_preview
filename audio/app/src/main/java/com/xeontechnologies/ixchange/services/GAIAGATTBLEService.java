...
import com.zitech.audio.gaia.UpgradeGaiaManager;
...
import com.zitech.audio.models.gatt.GattServiceHeartRate;
import com.zitech.audio.receivers.BondStateReceiver;
import com.zitech.audio.rwcp.RWCPClient;
...
public class GAIAGATTBLEService extends BLEService implements BondStateReceiver.BondStateListener,
    ...
    /**
     * <p>To set up the MTU size used between the connected device and the Android device.</p>
     * <p>This method will start the negotiations with the given size as the requested size. The maximum Android can is
     * {@link BLEService#MTU_SIZE_MAXIMUM MTU_SIZE_MAXIMUM}.</p>
     * <p>Response to this request is asynchronous and received via a {@link Messages#GATT_MESSAGE GATT_MESSAGE} message
     * with type {@link GattMessage#MTU_UPDATED MTU_UPDATED} and the new MTU size. If the operation wasn't
     * successful, the MTu size is then supposed as not supported and instead a
     * {@link Messages#GATT_MESSAGE GATT_MESSAGE} message with type {@link GattMessage#MTU_SUPPORTED MTU_SUPPORTED}
     * with false is sent.</p>
     *
     * @param size
     *          The MTU size to use when communicating with the connected device.
     *
     * @return True if the request could be queued.
     */
    public boolean setMtuSize(int size) {
    ...
    public int getRWCPInitialWindow() {
        return mRWCPClient.getInitialWindowSize();
    }

    /**
     * <p>To set up the initial window size for RWCP to the given size.</p>
     *
     * @param size
     ...
     */
    public boolean setRWCPInitialWindow(int size) {
        return mRWCPClient.setInitialWindowSize(size);
    }
    ...
    public int getRWCPMaximumWindow() {
        return mRWCPClient.getMaximumWindowSize();
    }

    /**
     * <p>To set up the maximum window size for RWCP to the given size.</p>
     *
     * @param size
     *          The size sets up by the user.
     *
     * @return True if it was possible to change the size, false otherwise.
     */
    public boolean setRWCPMaximumWindow(int size) {
        return mRWCPClient.setMaximumWindowSize(size);
    }
