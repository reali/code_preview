...
import com.zitech.audio.receivers.UUIDReceiver;
import com.zitech.audio.utils.Utils;
 ...
 * by default.</p>
 */
/*package*/ abstract class BREDRProvider implements UUIDReceiver.UUIDListener {
    ...
    /**
     * <p>A handler to run some tasks.</p>
     */
    private Handler mHandler = new Handler();
    /**
     * <p>To know when this provider is waiting for UUIDs to be fetched in order to connect to a device.</p>
     */
    ...
    @Override // UUIDReceiver.UUIDListener
    public void onUUIDFound(BluetoothDevice device, ParcelUuid[] uuids) {
        if (isWaitingForUUIDs && uuids != null && uuids.length > 0 && device != null) {
            UUID transport = getUUIDTransport(uuids);

            if (transport == null) {
                Log.w(TAG, "UUIDs found but nothing to match");
                return;
            }

            isWaitingForUUIDs = false; // cancels the related delayed task, see fetchUuidsWithSdp
     ...
     * through the {@link #connect(BluetoothDevice, Context) connect(BluetoothDevice, Context)} method.</p>
     *
     * @param address
     *              The device address to connect with.
     * @param context
     *              The application context to register a receiver if the UUIDs have to be fetched.
     *
     ...
     *
     * @param context
     *          The application context to connect to a device.
     *
     * @return <code>true</code> if the reconnection process could be initiated.
     */
    /*package*/ boolean reconnectToDevice(@NonNull Context context) {
        return mDevice != null && connect(mDevice, context);
    }
    ...
    /**
     * <p>This method checks if the given UUIDs correspond to one of the expected one for the connection.</p>
     * <p>This method expects for {@link UUIDs#SPP SPP} and {@link UUIDs#GAIA GAIA} UUIDs.</p>
     *
     * @param uuid
     *          The UUID to check.
     *
     * @return True if it is one of the expected UUIDs, false otherwise.
     */
     ...
     * @param context
     *          A non null context to register a receiver when fetching the device UUIDs.
     *
     * @return True if the connection had been initialised, false if it can't be started.
     */
    private boolean connect(final @NonNull BluetoothDevice device, @NonNull Context context) {
        if (mShowDebugLogs) {
        ...
        if (uuids == null) {
            Log.i(TAG, "No UUIDs found, starting fetch UUIDS with SDP procedure.");
            fetchUuidsWithSdp(device, context);
            return true;
        }
     ...
     * {@link BluetoothDevice#fetchUuidsWithSdp() fetchUuidsWithSdp} and delayed a task to unregister the receiver.</p>
     *
     * @param device
     *          The device to fetch UUIDs for.
     */
    private void fetchUuidsWithSdp(BluetoothDevice device, final Context context) {
        isWaitingForUUIDs = true;
        final UUIDReceiver receiver = new UUIDReceiver(this, device);
        ...
        mHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (isWaitingForUUIDs && getState() == State.CONNECTING) {
                    isWaitingForUUIDs = false;
                    Log.e(TAG, "Connection failed: no corresponding UUID found.");
                    setState(State.DISCONNECTED);
