...
public class RWCPClient {
    ...
    /**
     * <p>To get the current initial window size which is used to set up the window size when starting a session.</p>
     *
    ...
    /**
     * <p>To set up the initial window size when starting a RWCP session.</p>
     * <p>This can only be set up when there is no ongoing session.</p>
     *
     * @param size
     ...
     */
    public boolean setInitialWindowSize(int size) {
        logState("set initial window size to " + size);

        if (mState != RWCP.State.LISTEN) {
            Log.w(TAG, "FAIL to set initial window size to " + size + ": not possible when there is an ongoing " +
                    "session.");
            return false;
        }
    ...
    /**
     * <p>To get the current maximum window size which is used when adjusting the window size.</p>
     *
     * @return The initial window size.
     */
    ...
    /**
     * <p>To set up the maximum window size used when adjusting the window size during a session.</p>
     * <p>This can only be set up when there is no ongoing session.</p>
     *
     * @param size
     *          The size sets up by the user.
     ...
     * @return True if it was possible to change the size, false otherwise.
     */
    public boolean setMaximumWindowSize(int size) {
        logState("set maximum window size to " + size);

        if (mState != RWCP.State.LISTEN) {
            Log.w(TAG, "FAIL to set maximum window size to " + size + ": not possible when there is an ongoing " +
                    "session.");
            return false;
        }
        ...
        if (mWindow > mMaximumWindow) {
            Log.i(TAG, "window is updated to be less than the maximum window size (" + mMaximumWindow + ").");
     ...
     * <ul>
     *     <li>current window size and its default size,</li>
     ...
     * <blockquote><pre>label						RWCP state
     Window: 	current  = a 		default = b 		credits = c
     Sequence: 	last  = d 		next = e
     Pending: 	PSegments = f 		PData = g</pre></blockquote>
    ...
    private void logState(String label) {
        if (mShowDebugLogs) {
            String message = label + "\t\t\tstate=" + RWCP.getStateLabel(mState)
                    + "\n\tWindow: \tcurrent = " + mWindow + " \t\tdefault = " + mInitialWindow
                    + " \t\tcredits = " + mCredits
                    + "\n\tSequence: \tlast = " + mLastAckSequence + " \t\tnext = " + mNextSequence
                    + "\n\tPending: \tPSegments = " + mUnacknowledgedSegments.size()
