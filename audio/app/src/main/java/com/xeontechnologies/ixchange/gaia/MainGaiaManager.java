...
public class MainGaiaManager extends AGaiaManager {
    ...
    /**
     * <p>The total number of GAIA products listed in {@link Features Features}.</p>
     */
    ...
    /**
     * <p>To count how many of the commands to check had been tested to get the supported GAIA products.</p>
     */
    private int mCommandsChecked = 0;
    /**
     * <p>The number of commands to test to get the supported GAIA products.</p>
     */
    ...
    /**
     * <p>This regroups all products of the device which should be supported in this application. If at least one of
     ...
     * is supported.</p>
     * <p>The answer to this method is asynchronous. The listener will get a call for the products which are
     * supported with {@link MainGaiaManagerListener#onFeatureSupported(int) onFeatureSupported(int)}. Once all
     * products had been tested {@link MainGaiaManagerListener#onFeaturesDiscovered() onFeaturesDiscovered()} is
     * called.</p>
     */
     ...
     *          true if the LED should be activated, false if it should be disabled.
     */
    public void setViberationAlertState(boolean activate) {
        byte[] payload = activate ? PAYLOAD_BOOLEAN_TRUE : PAYLOAD_BOOLEAN_FALSE;
        GaiaPacket packet = createPacket(GAIA.COMMAND_SET_VIBRATOR_CONTROL, payload);
        createRequest(packet);
        Log.i(TAG, "Viberation : Payload setting to is "+activate );
    }
     ...
     *          true if the LED should be activated, false if it should be disabled.
     */
    public void setLEDAlertState(int activate) {
        byte[] payload = { 0x04 };
        GaiaPacket packet = createPacket(GAIA.COMMAND_ALERT_LEDS, payload);
        createRequest(packet);
        Log.i(TAG, "Viberation LED : Payload setting to is "+activate );
    }
    // ====== PROTECTED METHODS ====================================================================
    ...
    protected void receiveUnsuccessfulAcknowledgement(GaiaPacket packet) {
        ...
        else if(command == GAIA.COMMAND_ALERT_LEDS )
        {
            Log.i(TAG, "Viberation: Viberation feature not supported");
            onCommandChecked();
        }
        else {
     ...
     *          the received packet.
     */
    private void receiveGetViberatorControlACK(GaiaPacket packet) {
        ...
        if (payload.length >= PAYLOAD_MIN_LENGTH) {
            boolean activate = payload[PAYLOAD_VALUE_OFFSET] == 0x01;
            Log.i(TAG, "Viberation : Payload ACK is "+activate );
            mListener.onGetViberatorControl(activate);
        }
        else
        {
            Log.i(TAG, "Viberation : No ACK Payload" );
        }
    }
    /**
     * <p>This method is called when this manager handles an acknowledgement packet from the device with the command
     ...
     */
    private void receiveGetLEDControlACK(GaiaPacket packet) {
        Log.i(TAG, "Viberation : ACK Payload"+ packet.toString() );
        byte[] payload = packet.getPayload();
        final int PAYLOAD_VALUE_OFFSET = 0;
        final int PAYLOAD_VALUE_LENGTH = 1;
        final int PAYLOAD_MIN_LENGTH = PAYLOAD_VALUE_LENGTH + 1; // ACK status length is 1


        if (payload.length >= PAYLOAD_MIN_LENGTH) {
            boolean activate = payload[PAYLOAD_VALUE_OFFSET] == 0x00;
            if(payload[PAYLOAD_VALUE_OFFSET] ==0x00)
            {
                Log.i(TAG, "Viberation : Payload ACK is Success" );
            }
            else
            {
                Log.i(TAG, "Viberation : Payload ACK is invalid parameters" );
            }
    ...
    public interface MainGaiaManagerListener {
        ...
        /**
         * <p>To inform the listeners that all products have been tested after it has called
         ...
         *          True if the device has informed that the LED is on, false otherwise.
         */
        void onGetViberatorControl(boolean activate);
    }
