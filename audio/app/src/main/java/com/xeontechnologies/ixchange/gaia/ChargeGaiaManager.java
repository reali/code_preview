...
public class ChargeGaiaManager extends AGaiaManager {
    ...
    private final Runnable mRunnableBattery = new Runnable() {
        ...
        public void run() {
            synchronized (mPendingCustomNotifications) {
                if (mPendingCustomNotifications.containsKey(ChargeGaiaManager.Information.BATTERY)) {
                    mPendingCustomNotifications.put(ChargeGaiaManager.Information.BATTERY, true);
                    getInformation(ChargeGaiaManager.Information.BATTERY);
                }
            }
        }
    };
     ...
     */
    @IntDef(flag = true, value = { ChargeGaiaManager.Information.BATTERY, ChargeGaiaManager.Information.RSSI, ChargeGaiaManager.Information.API_VERSION, ChargeGaiaManager.Information.LED })
     ...
     */
    private final String TAG = "ChargeGaiaManager";
    ChargeGaiaManagerListener mListener;

    public ChargeGaiaManager(ChargeGaiaManagerListener mListener,int transportType) {
        super(transportType);
     ...
     * <p>Only the following {@link ChargeGaiaManager.Information} has available notifications: {@link ChargeGaiaManager.Information#BATTERY BATTERY}
     * and {@link ChargeGaiaManager.Information#RSSI RSSI}.</p>
    ...
    public void getNotifications(@ChargeGaiaManager.Information int information, boolean notify) {
        switch (information) {
            case ChargeGaiaManager.Information.BATTERY:
    ...
    private void getBatteryNotifications(boolean notify) {
        ...
        synchronized (mPendingCustomNotifications) {
            if (notify) {
                ...
                mPendingCustomNotifications.remove(ChargeGaiaManager.Information.BATTERY);
     ...
     */
    public void getInformation(@ChargeGaiaManager.Information int information) {
        switch (information) {
    ...
    protected boolean manageReceivedPacket(GaiaPacket packet) {
        ...
        Log.i(TAG, "manageReceivedPacket: ");
     ...
     *
     * @param event The notification event to cancel.
     */
    @SuppressWarnings("SameParameterValue")
     ...
     * </ul></p>
     *
     * @param packet The received notification event packet.
     ...
     *
     * @param packet The received packet with the GAIA command
     *               {@link GAIA#COMMAND_EVENT_NOTIFICATION COMMAND_EVENT_NOTIFICATION} and the event
    ...
    // the method has a return to be consistent with the method which called it
     ...
     *
     * @param packet The received packet with the command COMMAND_GET_CURRENT_BATTERY_LEVEL.
     */
    private void receivePacketGetCurrentBatteryLevelACK(GaiaPacket packet) {
        ...
        if (payload.length >= PAYLOAD_MIN_LENGTH) {
            ...
            synchronized (mPendingCustomNotifications) {
                if (mPendingCustomNotifications.containsKey(ChargeGaiaManager.Information.BATTERY)
                        && mPendingCustomNotifications.get(ChargeGaiaManager.Information.BATTERY)) {
                    // we received the waiting battery information
                    mPendingCustomNotifications.put(ChargeGaiaManager.Information.BATTERY, false);
    ...
    public interface ChargeGaiaManagerListener {
